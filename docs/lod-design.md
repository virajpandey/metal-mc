...
## Known gaps
- **Edges of explored areas.** Where the LOD has no data (unexplored chunks on a server, or ungenerated ones), neighboring terrain shows its cross-section down to the world bottom, like a floating slab. Voxy looks the same. Faces toward missing chunks could be dropped below the surface.
